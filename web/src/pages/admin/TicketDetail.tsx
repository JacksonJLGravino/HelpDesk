...
export function TicketDetail() {
  ...
  async function handleAssignTechnician() {
    ...
    setAssigning(true);
    try {
      await assignTechnician(ticket.id, selectedTechnicianId);
      const refreshed = await getTicket(ticket.id);
      setTicket(refreshed);
    } catch {
      alert("Não foi possível atribuir o técnico.");
    } finally {
      setAssigning(false);
    }
  }
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 flex items-center gap-2">
                <AvatarImg
                  ...
                  size="sm"
                />
                <span className="text-gray-200">{ticket.client?.name}</span>
              </div>
            </div>
          </div>
        </div>
